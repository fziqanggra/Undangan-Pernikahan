...
export const guest = (() => {
  ...
  const showGuestName = () => {
    ...
    if (name) {
      ...
      div.innerHTML = `<small class="mt-0 mb-1 mx-0 p-0">${guestName?.getAttribute(
        "data-message"
      )}</small><p class="m-0 p-0" style="font-size: 1.5rem">${util.escapeHtml(
        name
      )}</p>`;
  ...
  const buildGoogleCalendar = () => {
     ...
     */
    const formatDate = (d) =>
      new Date(d + ":00Z").toISOString().replace(/[-:]/g, "").split(".")[0];

    const url = new URL("https://calendar.google.com/calendar/render");
    const data = {
      action: "TEMPLATE",
      text: "The Wedding of Ijal and Hani",
      ...
      location:
        "https://www.google.com/maps?q=-6.6882401,106.8118001&z=17&hl=id",
      ctz: "Asia/Bogor",
    };
